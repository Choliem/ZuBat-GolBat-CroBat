...
function createTeeth(vertices, faces, color, zOffset) {
  ...
  for (const posX of positions) {
    ...
    faces.push(baseIndex, baseIndex + 1, baseIndex + 2, baseIndex, baseIndex + 2, baseIndex + 3);
  }
}
...
export class CrobatMouthAndTeeth extends SceneObject {
  constructor(GL, attribs) {
    ...
    createMouthShape(vertices, faces, MOUTH_WIDTH + OUTLINE_OFFSET, MOUTH_HEIGHT + OUTLINE_OFFSET, SEGMENTS, OUTLINE_COLOR, Z_OUTLINE, CURVE_AMOUNT);

    // STEP 2: Buat Mulut Putih (Normal, di tengah, bengkok)
    createMouthShape(vertices, faces, MOUTH_WIDTH, MOUTH_HEIGHT, SEGMENTS, MOUTH_COLOR, Z_MOUTH, CURVE_AMOUNT);

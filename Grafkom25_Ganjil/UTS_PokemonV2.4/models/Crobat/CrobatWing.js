...
function _generateDoubleSidedMembrane(
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    let vecForward = i < numPoints - 1 ? _subtractVectors(centerline1[i + 1], p1) : _subtractVectors(p1, centerline1[i - 1]);
    ...
    vertices.push(p1_front[0], p1_front[1], p1_front[2], ...innerColor, ...frontNormal);
    const p2_front = _addVectors(p2, frontOffset);
    vertices.push(p2_front[0], p2_front[1], p2_front[2], ...innerColor, ...frontNormal);
    const backOffset = _scaleVector(backNormal, halfThickness);
    const p1_back = _addVectors(p1, backOffset);
    vertices.push(p1_back[0], p1_back[1], p1_back[2], ...outerColor, ...backNormal);
    const p2_back = _addVectors(p2, backOffset);
    vertices.push(p2_back[0], p2_back[1], p2_back[2], ...outerColor, ...backNormal);
  }
  ...
  for (let i = 0; i < numPoints - 1; i++) {
    ...
    const fbr = fbl + numVerticesPerStrip;
    const btr = btl + numVerticesPerStrip;
    const bbr = bbl + numVerticesPerStrip;
...
function _generateTube(
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    else if (i === vertexCount - 1) tangent = _subtractVectors(point, path[i - 1]);
    else tangent = _normalizeVector(_addVectors(_subtractVectors(path[i + 1], point), _subtractVectors(point, path[i - 1])));
    ...
    for (let j = 0; j <= tubeSegments; j++) {
      ...
      const offset = _addVectors(_scaleVector(normal, Math.cos(angle) * currentThickness), _scaleVector(binormal, Math.sin(angle) * currentThickness));
      const vertexPos = _addVectors(point, offset);
      const vertexNormal = _normalizeVector(offset);

      vertices.push(vertexPos[0], vertexPos[1], vertexPos[2], ...color, vertexNormal[0], vertexNormal[1], vertexNormal[2]);
    }
  }
...
export class CrobatWing extends SceneObject {
  constructor(GL, attribs) {
    ...
    const MEMBRANE_TOP_CURVE = TOP_BONE_CURVE.map((segment) => ({
      startPoint: [segment.startPoint[0], segment.startPoint[1] - MEMBRANE_TOP_OFFSET, segment.startPoint[2]],
      startHandle: segment.startHandle,
      endPoint: [segment.endPoint[0], segment.endPoint[1] - MEMBRANE_TOP_OFFSET, segment.endPoint[2]],

...
export class ZubatLowerBody extends Node {
  ...
  _generateBodyGeometry(opts) {
    const vertices = [];
    const faces = [];
    const { radius, latBands, longBands, bodyColor, legTargets, legBaseCenters, legPullRadius, legPullSharpness } = opts;
    ...
    for (let latNumber = 0; latNumber <= latBands; latNumber++) {
      ...
      for (let longNumber = 0; longNumber <= longBands; longNumber++) {
        ...
        if (y < 0 && z < 0) {
          ...
          distFromBase = Math.sqrt(Math.pow(x - baseCenter.x, 2) + Math.pow(y - baseCenter.y, 2) + Math.pow(z - baseCenter.z, 2));
        ...
        const len = Math.sqrt(final_nx * final_nx + final_ny * final_ny + final_nz * final_nz);
  ...
  _applyBodyTransformation(opts) {
    ...
    LIBS.scale(scaleMatrix, 0.8 * scaleFactor, 0.8 * scaleFactor, 0.85 * scaleFactor);

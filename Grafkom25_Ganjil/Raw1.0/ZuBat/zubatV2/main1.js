...
function main() {
  ...
  // --- TAMBAHAN 3 (MENGGUNAKAN PREFERENSI ANDA) ---
  ...
  const animate = () => {
    ...
    // --- TAMBAHAN 5 (MENGGUNAKAN LOGIKA TERBAIK) ---
    // --- ANIMASI 4: Sonic Wave Attack (Otomatis) ---

    // 1. Spawn gelombang baru secara otomatis berdasarkan interval
    if (currentTime - lastWaveSpawnTime > waveSpawnInterval) {
      // Kita buat objek gelombang baru (cukup 1 cincin)
      const wave = new ZubatSonicWave(GL, attribs, waveOptions);

      // Simpan data untuk animasi
      wave.spawnTime = currentTime;
      wave.lifespan = waveLifespan;

      // Tambahkan ke scene graph (di bawah manager)
      sonicAttackManager.add(wave);
      activeWaves.push(wave); // Tambahkan ke array pelacak

      lastWaveSpawnTime = currentTime; // Reset timer spawn
    }
    ...
    for (let wave of activeWaves) {
      ...
      if (age > wave.lifespan) {
        ...
        // (Menggunakan Math.PI / 2, bukan Math.PI * 2, agar menghadap depan)

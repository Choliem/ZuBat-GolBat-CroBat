...
function main() {
  ...
  var cloudObj = Clouds.createSceneObject(GL, attribs, numClouds, skyRadius, skyHeight);
  ...
  const animatorNodes = [zubatAnimatorNode, golbatAnimatorNode, crobatAnimatorNode]; // [Zubat, Golbat, Crobat]
  ...
  var animate = function (time) {
    ...
    let activeBasePos = [activeBase.localMatrix[12], activeBase.localMatrix[13], activeBase.localMatrix[14]]; // Ambil posisi absolut center (base crobat, index 2)
    let centerPos = [baseNodes[2].localMatrix[12], baseNodes[2].localMatrix[13], baseNodes[2].localMatrix[14]]; // 3. Hitung 'progress' untuk stage saat ini
    ...
    switch (animationStage) {
      ...
      case "TURN_AND_BEND": {
        ...
        LIBS.translateX(activeNode.localMatrix, centerPos[0] - activeBasePos[0]);
        LIBS.translateY(activeNode.localMatrix, LIFT_Y_OFFSET);
        LIBS.translateZ(activeNode.localMatrix, centerPos[2] - activeBasePos[2] + ORBIT_RADIUS); // Terapkan rotasi
      ...
      case "CIRCLING": {
        ...
        LIBS.translateX(activeNode.localMatrix, centerPos[0] - activeBasePos[0] + circleX);
        LIBS.translateY(activeNode.localMatrix, LIFT_Y_OFFSET);
        LIBS.translateZ(activeNode.localMatrix, centerPos[2] - activeBasePos[2] + circleZ);
      ...
      case "PRE_RETURN_TURN": {
        ...
        LIBS.translateX(activeNode.localMatrix, centerPos[0] - activeBasePos[0]); // X di center
        LIBS.translateY(activeNode.localMatrix, LIFT_Y_OFFSET);
        LIBS.translateZ(activeNode.localMatrix, centerPos[2] - activeBasePos[2] + ORBIT_RADIUS); // Z di edge orbit // Rotasi: dari noleh samping (ORBIT_Y_ROTATION) -> noleh belakang (tambah PI)
      ...
      case "PRE_RETURN_MOVE": {
        ...
        LIBS.translateX(activeNode.localMatrix, centerPos[0] - activeBasePos[0]); // X tetap di center
        LIBS.translateY(activeNode.localMatrix, LIFT_Y_OFFSET);
      ...
      case "PRE_RETURN_TURN_FINAL": {
        ...
        LIBS.translateX(activeNode.localMatrix, centerPos[0] - activeBasePos[0]);
        LIBS.translateY(activeNode.localMatrix, LIFT_Y_OFFSET);
        LIBS.translateZ(activeNode.localMatrix, centerPos[2] - activeBasePos[2]); // Rotasi: LERP dari noleh belakang -> noleh depan (0), dan un-bend (X -> 0)
    ...
    // CLOUD ANIMATION
    const cloudDriftSpeed = 0.0001; // Seberapa cepat awan bergerak
    const cloudDriftAmount = 2000;  // Seberapa jauh awan bergerak (total jarak)
    
    // Hitung posisi X baru berdasarkan waktu
    let cloudDriftX = Math.sin(time * cloudDriftSpeed) * cloudDriftAmount;

    // Reset matriks lokal awan setiap frame
    LIBS.set_I4(cloudNode.localMatrix); 
    // Terapkan gerakan (translasi) pada sumbu X
    LIBS.translateX(cloudNode.localMatrix, cloudDriftX);
    // --- Akhir Animasi Awan ---
    cloudNode.draw(MOVEMATRIX, uniforms); // <-- BARIS ASLI (sekarang setelah animasi)
    // END CLOUD ANIMATION

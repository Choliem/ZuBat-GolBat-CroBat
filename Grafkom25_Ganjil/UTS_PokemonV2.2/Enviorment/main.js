// ===================================================================
// main.js - Menggunakan ES Modules
// ===================================================================

// 1. IMPORT SEMUA MODULES
// Path di-update untuk menunjuk ke 'models/'
...
function main() {
  ...
  if (!GL.getProgramParameter(SHADER_PROGRAM, GL.LINK_STATUS)) {
    ...
    return; // Hentikan eksekusi jika link gagal
  }
  ...
  // --- TAMBAHKAN POKEMON BASE INI ---
  const pokemonBaseRadius = 300;
  const pokemonBaseHeight = 50;
  const pokemonSpikeHeight = 200;
  const pokemonNumSpikes = 10;
  const pokemonBaseColor = [0.4, 0.4, 0.4]; // Warna abu-abu gelap
  const pokemonSpikeColor = [0.2, 0.2, 0.2]; // Warna abu-abu lebih gelap
  const pokemonBaseYPosition = waterData.waterLevel + 300; // Mengambang di atas air
  ...
  // --- TAMBAHKAN NODE POKEMON BASE INI ---
  var pokemonBaseNode1 = new Node();
  pokemonBaseNode1.setGeometry(pokemonBaseObj);
  LIBS.translateY(pokemonBaseNode1.localMatrix, 350);

  // Jika Anda ingin menggerakkan atau memutar basis, lakukan di pokemonBaseNode.localMatrix
  // Contoh: LIBS.rotateY(pokemonBaseNode.localMatrix, LIBS.degToRad(45));
  // --- AKHIR TAMBAH POKEMON BASE NODE ---

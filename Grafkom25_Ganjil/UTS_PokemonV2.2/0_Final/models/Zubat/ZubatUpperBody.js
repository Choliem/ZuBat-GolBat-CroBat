...
export class ZubatUpperBody extends Node {
  constructor(GL, attribs, options = {}) {
    ...
    const sceneObj = new SceneObject(GL, bodyData.vertices, bodyData.faces, attribs, "POS_COL_NORM");
  ...
  _generateBodyGeometry(opts) {
    const vertices = [];
    const faces = [];
    const { radius, latBands, longBands, bodyColor, mouthColor, mouthSurfaceCenter, mouthDepth, mouthSharpness, mouthControlPoints } = opts;

    // (Preprocess control points - tidak perlu komentar detail)
    const processedPoints = mouthControlPoints.map((p) => ({ ...p, angleRad: p.angle * (Math.PI / 180) })).sort((a, b) => a.angleRad - b.angleRad);
    ...
    for (let latNumber = 0; latNumber <= latBands; latNumber++) {
      ...
      for (let longNumber = 0; longNumber <= longBands; longNumber++) {
        ...
        if (z > 0.3) {
          ...
          if (t < 0 || t > 1) t = 0;
          const targetRadiusX = prevPoint.radii.x * (1 - t) + nextPoint.radii.x * t;
          const targetRadiusY = prevPoint.radii.y * (1 - t) + nextPoint.radii.y * t;
          const insideFactor = Math.pow(distX / targetRadiusX, 2) + Math.pow(distY / targetRadiusY, 2);
          ...
          if (insideFactor < blendEdgeEnd) {
            ...
            const targetPullX = prevPoint.pullTarget[0] * (1 - t) + nextPoint.pullTarget[0] * t;
            const targetPullY = prevPoint.pullTarget[1] * (1 - t) + nextPoint.pullTarget[1] * t;
            const targetPullZ = prevPoint.pullTarget[2] * (1 - t) + nextPoint.pullTarget[2] * t;
            ...
            const blendAmount = this._smoothstep(blendEdgeEnd, blendEdgeStart, insideFactor);

            // Blend warna dari biru ke hitam
            finalColor = [bodyColor[0] * (1.0 - blendAmount) + mouthColor[0] * blendAmount, bodyColor[1] * (1.0 - blendAmount) + mouthColor[1] * blendAmount, bodyColor[2] * (1.0 - blendAmount) + mouthColor[2] * blendAmount];
          }
        }

        // Normalisasi normal
        const len = Math.sqrt(final_nx * final_nx + final_ny * final_ny + final_nz * final_nz) || 1;
  ...
  _applyBodyTransformation(opts) {
    ...
    LIBS.scale(scaleMatrix, 0.7 * scaleFactor, 0.6 * scaleFactor, 0.7 * scaleFactor);

...
// --- TAMBAHAN 1 ---
import { ZubatSonicWave } from "./models/ZubatSonicWave.js";
...
function main() {
  ...
  // --- TAMBAHAN 2 ---
  // --- BAGIAN 4: MANAGER SERANGAN SONIC ---
  // Buat Node 'manager' untuk menampung semua gelombang yang di-spawn
  const sonicAttackManager = new Node();
  zubatModel.add(sonicAttackManager);

  LIBS.scale(zubatModel.localMatrix, 0.5, 0.5, 0.5);
  ...
  // --- TAMBAHAN 3 (FONDASI PATEN ANDA) ---
  // --- Variabel State untuk Animasi Sonic Wave ---
  let activeWaves = []; // Array untuk menyimpan gelombang yang aktif
  let lastWaveSpawnTime = 0; // Timer untuk spawn
  const waveSpawnInterval = 400; // Spawn gelombang baru setiap
  const waveLifespan = 2000; // Milidetik (2 detik)
  const waveMaxScale = 5.0; // Skala akhir
  const waveSpeed = 0.001; // Kecepatan gerak maju

  // Opsi untuk membuat 1 gelombang, menggunakan parameter dari kode statis Anda
  const waveOptions = {
    numRings: 1,
    ringSpacing: 0,
    baseRadius: 0.5, // <-- Menggunakan nilai Anda
    radiusGrowth: 0,
    tubeThickness: 0.01, // <-- Menggunakan nilai Anda
    mainSegments: 32,
    tubeSegments: 8,
  };

  // --- PERUBAHAN ---
  // Variabel state baru untuk trigger
  let isAutoAttacking = false; // Untuk 'Space'
  let isKey1Pressed = false; // Untuk '1'
  // --- AKHIR PERUBAHAN ---
  ...
  // --- Event Listeners untuk Keyboard (Trigger) ---
  window.addEventListener("keydown", (e) => {
    // Opsi B: Toggle "Space"
    if (e.key === " " || e.code === "Space") {
      isAutoAttacking = !isAutoAttacking; // Balik status on/off
    }

    // Opsi A: Tombol "1"
    if (e.key === "1") {
      // Jika tombol "1" sudah ditahan, abaikan event key-repeat
      if (isKey1Pressed) return;

      isKey1Pressed = true;
      // Sesuai permintaan: Tombol "1" akan mematikan ("cut") auto-attack "Space"
      isAutoAttacking = false;

      // Langsung spawn 1 gelombang untuk "click"
      spawnNewWave(performance.now());
    }
  });

  window.addEventListener("keyup", (e) => {
    // Opsi A: Tombol "1" dilepas
    if (e.key === "1") {
      isKey1Pressed = false; // Berhenti "hold"
    }
  });
  // --- AKHIR PERUBAHAN ---
  ...
  const animate = () => {
    ...
    // --- PERUBAHAN ---
    // --- ANIMASI 4: Sonic Wave Attack (Trigger baru) ---

    // 1. Cek trigger untuk spawn gelombang
    const timeSinceLastSpawn = currentTime - lastWaveSpawnTime;

    // Opsi B: "Space" auto-attack
    if (isAutoAttacking && timeSinceLastSpawn > waveSpawnInterval) {
      spawnNewWave(currentTime);
    }
    // Opsi A (Hold): "1" key hold
    // (Klik sudah ditangani keydown, ini menangani "hold")
    else if (isKey1Pressed && timeSinceLastSpawn > waveSpawnInterval) {
      spawnNewWave(currentTime);
    }
    // --- AKHIR PERUBAHAN ---

    // 2. Update dan bersihkan gelombang yang aktif
    const wavesToRemove = [];
    for (let wave of activeWaves) {
      const age = currentTime - wave.spawnTime; // Usia gelombang dlm ms

      if (age > wave.lifespan) {
        wavesToRemove.push(wave); // Tandai untuk dihapus
      } else {
        // Animasikan!
        // progress: 0.0 (baru) -> 1.0 (mati)
        const progress = age / wave.lifespan;

        // ANIMASI SCALE: Skala membesar seiring waktu
        // Mulai dari skala 1.0, berakhir di 'waveMaxScale'
        const currentScale = 0.1 + progress * (waveMaxScale - 1.0);

        // ANIMASI TRANSLATE: Bergerak maju (di sumbu Z lokal)
        const currentZ_anim = progress * waveSpeed;

        // --- PERBAIKAN TOTAL: Buat matriks secara manual ---
        // Alur transformasi yang benar: V' = Translate * Rotate * Scale * V

        // 1. Buat Matriks Skala (S)
        const M_scale = LIBS.get_I4();
        LIBS.scale(
          M_scale,
          currentScale, // Skala X (Radius)
          1.0, // Skala Y (Ketebalan) - TETAP 1.0
          currentScale // Skala Z (Radius)
        );

        // 2. Buat Matriks Rotasi (R)
        // (Menggunakan Math.PI * 2, sesuai fondasi Anda)
        const M_rotate = LIBS.get_I4();
        LIBS.rotateX(M_rotate, Math.PI * 2); // Arahkan ke depan

        // 3. Buat Matriks Translasi (T)
        // (Menggunakan fondasi posisi dari kode Anda)
        const M_translate = LIBS.get_I4();
        LIBS.translateY(M_translate, 2); // Posisi Y (dari fondasi Anda)
        LIBS.translateZ(M_translate, 3 + currentZ_anim); // Posisi Z + Gerakan

        // 4. Gabungkan: M_final = T * R * S
        const M_RotScale = LIBS.multiply(M_rotate, M_scale); // M_RS = R * S
        wave.localMatrix = LIBS.multiply(M_translate, M_RotScale); // M_final = T * (R * S)
      }
    }

    // 3. Hapus gelombang yang sudah mati (ANIMASI HILANG)
    if (wavesToRemove.length > 0) {
      // Filter array 'childs' di manager
      sonicAttackManager.childs = activeWaves.filter(
        (w) => !wavesToRemove.includes(w)
      );
      // Update array pelacak

...
export class ZubatTooth extends Node {
  ...
  _generateToothGeometry(opts) {
    ...
    // Loop dari tip (y=height) ke base (y=0)
    for (let j = 0; j <= rings; j++) {
      const y = (j / rings) * height; // Y position dari top ke bottom
      const t = j / rings; // Normalized position [0,1]
    ...
    vertices.push(xOffset, height, 0); // Position at tip (with curve offset)
    vertices.push(...color);
